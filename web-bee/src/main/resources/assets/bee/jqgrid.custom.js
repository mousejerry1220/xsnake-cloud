	...
	this.init = function(){
		...
		$(window).on('resize.jqGrid', function () {
    		...
    		if(newOption.buttons && newOption.buttons.length > 0){
    			...
    			var btn = $("<button id='"+e.id+"' style='margin-left: 2px; margin-right: 2px;' class='btn btn-info btn-xs btn-bold'> <i class='ace-icon fa fa-floppy-o bigger-120 blue'></i> 新增 </button>")
    				btn.click(function(){
    					$("#load_"+gridId).show();
	    ...
	    var defaultOption = {
	            ...
	            beforeEditCell:function(rowid,cellname,value,iRow,iCol){
	            	currentCell = {'rowid':rowid,'cellname':cellname,'value':value};
			...
			custom_element : function(value, options) {
				...
				$(el).beeChoice({
					...
					callback:function(r){
						$(e).html(r.name);
						debugger;
						$(this).setCell(options.rowId,options.idKey,r.id)
					},
					height:"300px",
					multiple:false,
					selectedList:[{id:'5dfd'}],
					noBackdrop:true
				});
				return el;
			},
			custom_value : function(elem, operation, value) {
				var result = $(elem).find('label').html();
				return result;
			}
...
//if($(this).getColProp(cellname).editoptions){
//	var idKey = $(this).getColProp(cellname).editoptions.idKey;
